...
Then read the row for `threadradar.xyz`. Or pull from
...
## 2026-05-31 — The wedge is that fractional CMOs are an underserved, high-intent,…
- **Status:** active
- **Hypothesis:** The wedge is that fractional CMOs are an underserved, high-intent, multi-seat buyer — each customer represents 3–5 brand subscriptions, not one — and they cluster in identifiable communities that respond to peer recommendation. The growth loop is: free pulse tool drives email signups → cohort onboards at $150/brand → briefings get forwarded to clients (who are themselves potential referrers or future ICPs) → forwarded briefings drive direct inbound. Programmatic SEO on `/pulse/[community]` pages compounds organic discovery while the free tool establishes credibility. If the briefing artifact is genuinely good enough to forward unedited — the core product bet — the loop is self-sustaining without paid acquisition.
- **KPI:** any GSC traffic — clicks, impressions, indexed-page count
- **Baseline:** 0 clicks / 0 impressions (just deployed)
- **Action:** project scaffolded via `portfolio new bootstrap`; first deploy pending. After deploy: verify in GSC as `sc-domain:threadradar.xyz` and submit the sitemap.
- **Result:** TBD — review 2026-06-28
- **Learning:** TBD
